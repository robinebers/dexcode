...
const LONG_CONTEXT_BETA_RE = /^(?:context-1m|long-context)/i;

function stripContextWindowSuffix(model) {
  return typeof model === "string" ? model.replace(CONTEXT_SUFFIX_RE, "").trim() : model;
}

function stripLongContextBetas(headers) {
  const out = { ...headers };
  for (const key of Object.keys(out)) {
    if (key.toLowerCase() !== "anthropic-beta") continue;
    const kept = String(Array.isArray(out[key]) ? out[key].join(",") : out[key])
      .split(",")
      .map((value) => value.trim())
      .filter((value) => value && !LONG_CONTEXT_BETA_RE.test(value));
...
function normalizeModel(model) {
  const value = stripContextWindowSuffix(String(model || "gpt-5.4").trim());
  const aliases = {
    sol: "gpt-5.6-sol",
    terra: "gpt-5.6-terra",
    luna: "gpt-5.6-luna",
...
function classifierBody(body) {
  // Auto-mode classifiers route exactly like normal requests (GPT slugs →
  // Codex, claude-* → Anthropic). Only the [1m] context suffix and the
  // querySource markers are stripped: Anthropic returns 400 when a classifier
  // model is paired with the long-context beta on subscriptions without it,
  // which made Claude Code fail closed on auto mode.
  const out = { ...body, model: stripContextWindowSuffix(body.model) };
...
function anthropicToCodexBody(body, { maxEffort = "high" } = {}) {
  ...
  // backend used here currently rejects it. GPT-routed auto-mode classifier
  // requests bound their output via the </block> stop-sequence filter instead
  // of their small max_tokens limit.
  if (tools.length) {
    out.tools = tools;
...
async function proxyAnthropic(
  ...
  body,
  context,
  { sanitizeLongContext = false, logErrorBody = false } = {},
) {
  let headers = copyReqHeaders(req);
  if (sanitizeLongContext) headers = stripLongContextBetas(headers);
  ...
  try {
    ...
    if (logErrorBody && (response.statusCode || 0) >= 400) {
      const chunks = [];
      for await (const chunk of response) chunks.push(chunk);
      const errorBody = Buffer.concat(chunks);
      log(`anthropic upstream ${response.statusCode}: ${errorBody.toString("utf8").slice(0, 300)}`);
      finishRequest(context, "response", {
        status: response.statusCode,
        errorBody: errorBody.toString("utf8").slice(0, 500),
      });
...
async function handleMessages(req, res, options) {
  ...
  const classifier = isAutoModeRequest(body);
  const routed = classifier ? classifierBody(body) : body;
  const kind = classifier ? "classifier" : "request";
  if (isClaudeModel(routed.model)) {
    const context = requestContext(options.logger, routed, classifier ? "classifier" : "anthropic");
    log(`${kind} → ANTHROPIC model=${JSON.stringify(routed.model || "")}`, { debug: true });
    await proxyAnthropic(req, res, routed, context, {
      sanitizeLongContext: classifier,
      logErrorBody: classifier,
    });
  } else {
    const context = requestContext(options.logger, routed, classifier ? "classifier" : "codex");
    log(`${kind} → CODEX/GPT model=${JSON.stringify(routed.model || "")}`, { debug: true });
    await proxyCodex(req, res, routed, options, context);
  }
}
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === "GET" && ["/", "/health", "/healthz"].includes(pathname)) {
        sendJson(res, 200, { ok: true, service: "situationship" });
        return;
      }
